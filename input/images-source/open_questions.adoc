...
. FHIRcast requires the hub to automatically send the last *-open context event to all new subscribers.
    * Alternatively, my preference is that we can require the subscribers to always send a Get Context Event [RAD-151] request upon initial subscription complete to get the latest context and content. This is newly introduced with the DiagnosticReport events.
